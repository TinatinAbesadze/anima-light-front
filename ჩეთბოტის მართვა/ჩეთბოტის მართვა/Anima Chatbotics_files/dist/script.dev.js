...
$(document).on('click', function (event) {
  if ($(event.target).hasClass('trigger-vid') && !$(".show-tutorial").hasClass('show')) {
    ...
    if ($(event.target).hasClass("chat-anima-btn")) {//do nothing, ეს ხელს უშლის დაელაპარაკე ანიმას ფბ ჩეთს
    } else {
...
var scrolledItem = document.querySelector(".bot-dialog-entry p");
var arrowDown = document.querySelector(".entry-angle-down");

if (window.location.pathname === "/profile:dialogs.html") {
  // onlyworks on this page
  for (var i = 0; i < 10; i++) {
    if (scrolledItem.scrollHeight > 40) {
